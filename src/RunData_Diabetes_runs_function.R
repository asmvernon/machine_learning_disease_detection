...
RunData_Diabetes_runs_function = function(PATHS, Title="",DWT,Samples="",fromRun=1,
                                          toRun=4,titles="",PCA= FALSE, exclude=0,subset=FALSE,
...
# # source('src/titles_fun.R')
  ...
  ##----------------------------------------------------------------------
  input<- list(Title,DWT,Samples,fromRun,toRun,PCA,runModels,exclude,subset,nKeep,featMode,Direction,nfeats,dimensions,cropped)
  names(input)<- c("Title","DWT","Samples","fromRun","toRun","PCA","runModels","exclude",
  ...
  for (m in fromRun:toRun){ 
    ...
    if(is.null(demoData)||(!is.null(demoData)&&!only_demoData)){
      dataPattern <- paste("export_matrix_000",m,sep="")
      ...
      for (i in 1:nDataItems){
        currentString <- strsplit(dataFiles[i], PATHS$dataPath)[[1]][2]
      ...
      dataFiles       <- dataFiles[keep_dataItems]
      sampleIDs       <- sampleIDs[keep_dataItems]
      nDataItems      <- length(targetValues)
      ##----------------------------------------------------------------------
      ## READ IN THE DATA ----------------------------------------------------
      ##----------------------------------------------------------------------
      data.input            <- ReadInFaimsData(dataFiles) 
      #### RUN ENSEMBLE
      # data1 <- data.input
      # data2 <- data.input
      # data3 <- data.input
      # data_mean <- matrix(0,dim(data1)[1],dim(data1)[2])
      # for(i in 1:ncol(data_mean)) data_mean[,i] <- apply(cbind(data1[,i],data2[,i],data3[,i]),1,mean)
      # data.input <- data_mean
  ...
  ##----------------------------------------------------------------------
  ## PLOT EACH DATA ITEMS TO SCREEN --------------------------------------
  ##----------------------------------------------------------------------
  # for (i in 1:nDataItems){
  #  plotData<- PlotFaimsData(data.input[i,], sampleIDs[i])
  #  png(paste(sampleIDs[i],'.','png',sep=""))
  #  dev.off()
  #  readline("Press <return to continue>")
  # }
  ##----------------------------------------------------------------------
  ## PLOT WAVELET TRASNFORMED DATA TO SCREEN -----------------------------
  ##----------------------------------------------------------------------
  # for (i in 1:nDataItems){
  #   png(paste(sampleIDs[i],'_data','.png',sep=""))  
  #   plotData<- PlotFaimsData(data.input[i,], sampleIDs[i])
  # #   plot(data.input[i,],type='l',main=paste("Data",sampleIDs[i]),xlab= "Feature",ylab = "Compensation Voltage")
  #   dev.off()
  #   pdf(paste(sampleIDs[i],Title,'.pdf',sep=""))
  #   plot(data.wd[i,],type='l',main=paste("Wavelet Decomposition",sampleIDs[i]),xlab= "Feature",ylab = "Frequency")
  #   dev.off()
  #   #  readline("Press <return to continue>")
  #   }
  ##*****************************************************************************
